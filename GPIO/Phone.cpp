...
int OnDeviceBufReceive(std::string buf, void* opaque) {
	return (reinterpret_cast<GpioEt7000*>(opaque))->OnDeviceBufRx(buf);
}

void OnPeriodicPoll(void* opaque) {
	(reinterpret_cast<GpioEt7000*>(opaque))->OnPoll();
}
...
void __stdcall SetCallbacks(void *cookie, CALLBACK_LOG lpLog, CALLBACK_CONNECT lpConnect, CALLBACK_KEY lpKey) {
    ...
    lpLogFn(callbackCookie, "Plugin DLL for ET-7000 loaded\n");
}
...
int __stdcall Connect(void) {
	...
	device.Configure(appSettings.Device.asAddress.c_str(), appSettings.Device.port,
		GpioEt7000::initBuf, GpioEt7000::KEEPALIVE_PERIOD, GpioEt7000::keepAliveBuf);
	return device.Start();
}
